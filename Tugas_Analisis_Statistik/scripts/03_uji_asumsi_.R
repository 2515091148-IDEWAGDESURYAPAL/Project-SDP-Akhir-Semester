...
"Pendapatan_Tahunan_Miliar_IDR" <- "nama_kolom_numerik"
...
ggsave(
  filename = paste0("../results/qqplot_","Pendapatan_Tahunan_Miliar_IDR", ".png"),
  plot = qq_plot,
  width = 8,
  height = 6
)
...
if(nrow(data_bersih) < 5000) {
  ...
  # Ambil kolom numerik yang akan diuji
  kolom_uji <- data$Pendapatan_Tahunan_Miliar_IDR
  
  # Hapus NA jika ada
  kolom_uji <- na.omit(kolom_uji)
  ...
  if (n <= 5000) {
    ...
    shapiro_test_result <- shapiro.test(kolom_uji)
    ...
    if (p_value > 0.05) {
      print(paste(
        ...
        round(p_value, 4),
        "> 0.05. Data kemungkinan besar terdistribusi normal."
      ))
    } else {
      print(paste(
        "Interpretasi: p-value =",
        round(p_value, 4),
        "<= 0.05. Data kemungkinan besar TIDAK terdistribusi normal."
      ))
    }
    
  } else {
    print(
      "Jumlah data lebih dari 5000, uji Shapiro-Wilk mungkin tidak akurat.
     Pertimbangkan uji lain seperti Kolmogorov-Smirnov atau inspeksi visual (Q-Q Plot)."
    )
  }
  
  # Pesan akhir
  print("Uji normalitas selesai. Gunakan hasil ini untuk menentukan apakah analisis parametrik sesuai.")
