...
if(!"Pendapatan_Tahunan_Miliar_IDR" %in% names(data_bersih) || !"Nilai_Pelanggan_Juta_IDR" %in% names(data_bersih)) {
  stop(paste("Satu atau kedua kolom ('",  "Pendapatan_Tahunan_Miliar_IDR", "', '", "Nilai_Pelanggan_Juta_IDR", "') tidak ditemukan. Periksa kembali nama kolom pada file 04_analisis_korelasi.R"))
}
...
scatter_plot <- ggplot(data_bersih, aes_string(x =  "Pendapatan_Tahunan_Miliar_IDR", y =  "Nilai_Pelanggan_Juta_IDR")) +
...
ggsave(
  filename = paste0("../results/scatterplot_","Pendapatan_Tahunan_Miliar_IDR" , "_vs_", "Nilai_Pelanggan_Juta_IDR", ".png"),
  plot = scatter_plot,
  width = 8,
  height = 6
)
...
correlation_test <- cor.test(data_bersih[["Pendapatan_Tahunan_Miliar_IDR"]], data_bersih[["Nilai_Pelanggan_Juta_IDR"]], method = "pearson")

print(paste("--- Hasil Uji Korelasi antara", "Pendapatan_Tahunan_Miliar_IDR", "dan", "Nilai_Pelanggan_Juta_IDR", "---"))
...
cat(
  ...
  "dan",
  strength,
  "antara Pendapatan_Tahunan_Miliar_IDR dan Nilai_Pelanggan_Juta_IDR.\n"
)
